...
from api.lib.state import State, DbtRunLogs
...
def test_dbt_run_logs_init_log_file():
    run_logs = DbtRunLogs('test_uuid')
    with patch('api.lib.state.CLOUD_STORAGE_INSTANCE.write_file') as mock_write_file:
        run_logs.init_log_file()
        mock_write_file.assert_called_once()

def test_dbt_run_logs_get():
    run_logs = DbtRunLogs('test_uuid')
    with patch('api.lib.state.CLOUD_STORAGE_INSTANCE.get_file') as mock_get_file:
