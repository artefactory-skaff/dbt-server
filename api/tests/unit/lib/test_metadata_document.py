...
from api.lib.metadata_document import MetadataDocument, FirestoreDocument, CosmosDBDocument, MetadataDocumentFactory
...
@patch('api.lib.metadata_document.firestore')
def test_firestore_document_methods(mock_firestore):
    ...
    doc.create({'key': 'value'})
    mock_firestore.Client().client.collection().document().set.assert_called_once_with({'key': 'value'})
    doc.update({'key': 'value'})
    mock_firestore.Client().client.collection().document().update.assert_called_once_with({'key': 'value'})

@patch('api.lib.metadata_document.CosmosClient')
@patch('api.lib.metadata_document.settings')
def test_cosmos_db_document_methods(settings, mock_client):
    ...
    doc.create({'key': 'value'})
    mock_client().get_database_client().get_container_client().upsert_item.assert_called_once_with(body={'key': 'value'})
    doc.update({'key': 'value'})
