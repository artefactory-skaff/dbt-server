...
from api.lib.command_processor import (
    process_command, get_sub_command_name, get_command_args_list,
    override_command_params, get_sub_command_click_context,
    get_sub_command_args_list, override_sub_command_params,
    get_sub_command_default_params, get_args_list_from_params_dict,
    get_key_differences_between_dict, get_arg_list_from_param,
...
def test_process_command():
    ...
    assert processed_command == '--debug --log-format json --log-level debug list --profiles-dir . --project-dir .'
...
def test_get_command_args_list():
    args_list = ['--log-format', 'text', '--log-level', 'debug', 'run', '--select', 'vbak_dbt']
...
def test_override_command_params():
    ...
    assert override_command_params(params) == {'log_format': 'json', 'log_level': 'debug', 'debug': True, 'quiet': False}

def test_get_sub_command_click_context():
    args_list = ['--wrong-flag', 'test', '--log-format', 'text', '--log-level', 'debug', 'run', '--select', 'vbak_dbt']
...
def test_override_sub_command_params():
    ...
    assert override_sub_command_params(args) == {'profiles_dir': '.', 'project_dir': '.'}
...
def test_get_args_list_from_params_dict():
    ...
    assert get_args_list_from_params_dict(command_key_diff, command_params_dict) == ['--profiles-dir', '.']
...
def test_get_arg_list_from_param():
    assert get_arg_list_from_param('select', ('model1', 'model2')) == ['--select', 'model1', 'model2']
