...
def test_init_state(MockCloudStorage, MockState):
    ...
    mock_gcs_client.write_to_bucket.assert_called_once_with('dbt-server-test',
...
def test_get_last_logs(MockCloudStorage, MockState):
    ...
    mock_gcs_client.get_blob_from_bucket.assert_called_once_with('dbt-server-test', f'logs/{uuid}.txt', 0)
...
def test_log(MockCloudStorage, MockState):
    ...
    mock_gcs_client.write_to_bucket.assert_called_once_with('dbt-server-test', f'logs/{uuid}.txt', new_log)
