...
from package.src.dbt_remote.server_response_classes import (
    DbtResponseLogs,
...
def get_link_from_action_name(links: List[FollowUpLink], action_name: str) -> str:
    ...
    raise click.ClickException(
        "Error in parsing server response: no link for action name {action_name}"
    )
...
def get_run_status(run_status_link: str) -> DbtResponseRunStatus:
    ...
    try:
        ...
        raise click.ClickException(
            "Error in parsing: " + traceback_str + "\n Original message: " + res.text
        )
